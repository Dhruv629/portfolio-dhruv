              ...
              <div className="flex flex-wrap justify-center gap-12">
                {section.skills.map((skill) => {
                  ...
                  return (
                    <div
                      key={skill.name}
                      className="relative group w-44 h-44 flex flex-col items-center justify-center bg-slate-700/90 rounded-2xl shadow-lg cursor-pointer transition-transform duration-300 hover:scale-110 animate-float"
                      style={{ animationDelay: `${Math.random() * 1.5}s` }}
                    >
                      <div className="text-5xl mb-3 text-cyan-400 drop-shadow-lg animate-dangle">
                        <Icon />
                      </div>
                      <span className="text-sm text-white font-bold text-center">{skill.name}</span>
                      <div className="absolute left-1/2 -translate-x-1/2 bottom-[-3rem] w-56 p-3 rounded-lg bg-white text-xs text-slate-800 shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 z-[9999] text-center font-semibold border border-slate-300">
                        {skill.desc}
                      </div>
                    </div>
                  );
