              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 justify-items-center">
                {section.skills.map((skill) => {
                  ...
                  return (
                    <div
                      key={skill.name}
                      className="relative group w-32 h-32 md:w-44 md:h-44 flex flex-col items-center justify-center bg-white/90 dark:bg-slate-700/90 rounded-2xl shadow-lg cursor-pointer transition-transform duration-300 hover:scale-110 animate-float"
                      style={{ animationDelay: `${Math.random() * 1.5}s` }}
                    >
                      <div className="text-3xl md:text-5xl mb-2 md:mb-3 text-cyan-600 dark:text-cyan-400 drop-shadow-lg animate-dangle">
                        <Icon />
                      </div>
                      <span className="text-xs md:text-sm text-slate-800 dark:text-white font-bold text-center">{skill.name}</span>
                      <div className="absolute left-1/2 -translate-x-1/2 bottom-[-3rem] w-40 md:w-56 p-2 md:p-3 rounded-lg bg-white dark:bg-slate-800 text-xs text-slate-800 dark:text-slate-300 shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 z-[9999] text-center font-semibold border border-slate-300 dark:border-slate-700">
                        {skill.desc}
                      </div>
                    </div>
                  );
